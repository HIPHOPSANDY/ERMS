...
function LoadingAttendance() {
	...
	$sql  = "SELECT `emp`.`emp_code`, `emp`.`first_name`, `emp`.`last_name`, `att`.`attendance_id`, `att`.`emp_code`, `att`.`attendance_date`, GROUP_CONCAT(`att`.`action_time`) AS `times`";
	$sql .= " FROM `" . DB_PREFIX . "employees` AS `emp`, `" . DB_PREFIX . "attendance` AS `att` WHERE `emp`.`emp_code` = `att`.`emp_code`";
	if ( !empty($requestData['search']['value']) ) {
		// $sql .= " OR `att`.`emp_code` LIKE '" . $requestData['search']['value'] . "%')";
...
function LoadingEmployees() {
	...
	if ( !empty($requestData['search']['value']) ) {
		// $sql .= " OR `emp_code` LIKE '" . $requestData['search']['value'] . "%')";
		$sql .= " AND (`emp_code` LIKE '" . $requestData['search']['value'] . "%'";
